...
| Report absolute lift and uncertainty. | Report relative percentage improvement alone. | Relative lift can exaggerate small changes and hides statistical uncertainty. |
| Use pre-treatment segments only. | Segment by activation status. | Segmenting on a post-treatment outcome creates selection bias. |
| Treat the dashboard as a decision tool. | Add every possible chart. | Each visual must answer a rollout, driver, guardrail, or targeting question. |
